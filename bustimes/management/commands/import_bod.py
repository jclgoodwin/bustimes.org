...
class Command(BaseCommand):
    ...
    def handle(self, api_key, **options):
        ...
        for operator_id, region_id in settings.BOD_OPERATORS:
            command.operators = {
                'CO': operator_id
            }
            ...
            print(Route.objects.filter(service__operator=operator_id).exclude(source__in=sources).delete())
            print(Service.objects.filter(operator=operator_id, current=True, route=None).update(current=False))
