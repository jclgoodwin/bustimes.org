...
class Command(BaseCommand):
    ...
    def handle(self, api_key, **options):
        ...
        trips = trips.annotate(pattern=StringAgg("stoptime__stop", ":"))
        ...
        for trip in trips:
            ...
            stop_times = trip.stoptime_set.filter(
    ...
    def handle_trip(self, session, url, trip, stop_times):
        ...
        for match in [response["trip"]] + [
            alt["trip"] for alt in response["alternates"]
        ]:
            ...
            for from_index in range(
                from_location["original_index"], to_location["original_index"]
            ):
