...
class Command(ImportFromCSVCommand):
    ...
    def is_rubbish_name(name):
        ...
        return (
            name
            in {"First", "Arriva", "Stagecoach", "Oakwood Travel", "Arriva North West"}
            or name.startswith("inc.")
            or name.startswith("formerly")
            or name.isupper()
        )
    ...
    def handle_row(self, row):
        ...
        if operator_id in self.removed_operator_ids:
            return
        operator_name = (
            self.get_name(row).replace("'", "\u2019").strip()
        )  # Fancy apostrophe
        ...
        if row["Licence"]:
            try:
                operator.licences.add(
                    Licence.objects.get(licence_number=row["Licence"])
                )
