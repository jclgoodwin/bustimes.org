...
class Command(ImportLiveVehiclesCommand):
    ...
    async def send_items(self, items):
        await get_channel_layer().send(
            "sirivm", {"type": "sirivm", "items": items, "when": self.when}
        )
    ...
    def update(self):
        ...
        for item in items:
            ...
            if (
                key not in self.identifiers
                or self.identifiers[key] < item["RecordedAtTime"]
            ):
                if key in self.identifiers:
                    assert parse_datetime(self.identifiers[key]) < parse_datetime(
                        item["RecordedAtTime"]
                    )
