...
def get_livery_choices(operator, vehicle, user):
    ...
    liveries = Livery.objects.filter(Q(vehicle__in=vehicles) | Q(operator=operator, published=True))
...
can’t be contradicted"""
        ...
        if not (user.is_staff or vehicle.branding or vehicle.operator_id == 'TNXB' or vehicle.operator_id == 'TCVW'):
